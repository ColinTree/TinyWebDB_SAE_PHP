...
if($kv->get('tinywebdbMANAGE_password')!=''){exit('<script>window.location.href="?a=index";'.((isset($_GET['de']))?($kv->delete(''.$_GET['de'])):'').'</script>');}
if(isset($_REQUEST['pwd'])){
    if(strlen($_REQUEST['pwd'])>=6){
        $kv->set('tinywebdbMANAGE_password',$_REQUEST['pwd']);
...
?><div class="panel panel-default"><?
  ...
  ?><div class="panel-body"><?
    ?><p><b>请创建用于后台的密码：</b></p><?
    if(count($kv->pkrget(prefix,1))>0){
      ?><p style="color:gray">请放心，本次重新创建密码不影旧版本的数据库内容<br>而原本的管理密码现在已经作废，引起的不便，敬请谅解</p><?
    }
    ?><form action="?a=init" method="post"><?
      ?><div class="input-group"><?
        ?><input autocomplete="off" type="password" class="form-control" name="pwd"><?
        ?><span class="input-group-btn"><input class="btn btn-default" type="submit" value="确定"></span><?
      ?></div><?
    ?></form><?
