<?
define('MANAGEversion','201704281');
...
switch($a){
	...
	case'all': check_login(); ?><div id="all_processing_background" style="display:none"><div class="panel panel-default"><div class="panel-body text-center"><h4 id="all_processing_msg1">MSG1</h4><h4 id="all_processing_msg2" style="margin:0">MSG2</h4></div></div></div><div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">全部</h3></div><div class="panel-body text-center"><div id="all_toolbar_frame" class="panel panel-default text-center" style="display:none">操作：<button id="all_toolbar_selectall" class="btn btn-primary">全选</button><button id="all_toolbar_selectnone" style="display:none" class="btn btn-primary">全不选</button><button id="all_toolbar_delete" class="btn btn-danger">删除</button></div><table class="table table-hover"><thead><th></th><th>标签-值</th><th>操作</th></thead><tbody><?$ret=$kv->pkrget(prefix,100);while(true){foreach($ret as $k=>$v){$showTag=htmlspecialchars(substr($k,strlen(prefix)));$showValue=substr(str_replace("\n",'',htmlspecialchars($v)),0,120);echo'<tr>', '<td><input type="checkbox" tinywebdb_key="',$showTag,'"></td>', '<td>',$showTag,'<br><div class="tinywebdb-value">',$showValue,'</div></td>', '<td><a href="?a=get&k=',urlencode_plus(substr($k,strlen(prefix))),'" class="btn btn-primary">查看</a><a href="?a=set&k=',urlencode_plus(substr($k,strlen(prefix))),'" class="btn btn-primary">修改</a><a href="?a=del&k=',urlencode_plus(substr($k,strlen(prefix))),'" onclick="return confirm(\'确认删除？\');" class="btn btn-danger">删除</a></td>', '</tr>';}end($ret);$start_key=key($ret);$i=count($ret);if($i<100)break;$ret=$kv->pkrget(prefix,100,$start_key);}?></tbody></table></div></div><? break;
    case'mdelete': check_login(); if(isset($_POST['tags'])){foreach($_POST['tags'] as $tag){$kv->delete(prefix.$tag);}}exit('finish'); break;
}
